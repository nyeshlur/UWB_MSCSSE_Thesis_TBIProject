...
bad_patients = ['DoD027', 'DoD028', 'DoD035',
                'DoD036', 'DoD038', 'DoD049', 'DoD069', 'DoD090']
...
def standardize(displacement, x_dim, y_dim):
    ...
    """
    displacement = cv2.resize(displacement, (y_dim, x_dim)) # bilinear by default
...
def extract_single_frame_from_displacementNorm(rawData, cardiac_cycle=0, frame=7):
    """
...
def process_one_patient(path, x_dim, y_dim, objective):
    ...
    for fileName in os.listdir(path):
        if ".mat" in fileName:
            ...
            displace_data, bMode, label = process_one_file(
                filePath, x_dim, y_dim, objective)
...
def process_all_patients(path, objective=1, x_dim=256, y_dim=80, patient_nums=None):
    ...
    if patient_nums is None:
        patient_nums = [patient for patient in os.listdir(
            path) if patient not in bad_patients]
...
def make_hdf5(file_name, save_path, data_path):
    ...
    """
    # process data
    displacement, label, bMode, fileNames = process_all_patients(
        data_path, objective=1, patient_nums=None)
...
def make_hdf5_by_patient_group(file_name, save_path, data_path, objective):
    ...
    all_patients = [patient for patient in os.listdir(
        data_path) if patient not in bad_patients]
